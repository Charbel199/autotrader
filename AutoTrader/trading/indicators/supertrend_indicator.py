...
class SuperTrend(Indicator):
    # columns = ['Time', 'TrueRange', 'ATR','BasicUpperBand','BasicLowerBand','FinalUpperBand','FinalLowerBand','Supertrend]
    period = 10
    multiplier = 3
    ...
    def get_plot(self) -> List:
        return [go.Scatter(x=[d['Time'] for d in self.list],
                           y=[d['Supertrend'] if 'Supertrend' in d else None for d in self.list],
                           name="Supertrend")]
